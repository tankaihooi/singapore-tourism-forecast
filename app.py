...
# ── Optional heavy imports — graceful fallback for HF Spaces cold starts ──────
try:
...
<style>
    ...
    .section-header { font-size: 15px; font-weight: 600; color: #333; margin: 24px 0 8px; color: white; }
    .note-box {
        background: #fff8e1;
...
# Colours for uncertainty bands
BAND_ALPHA = 0.18


@st.cache_data(show_spinner=False)
def load_arrivals() -> pd.DataFrame:
    """Load cleaned arrivals or generate synthetic demo data."""
    path = DATA_DIR / "arrivals_clean.csv"
    if path.exists():
        df = pd.read_csv(path, parse_dates=["date"])
        return df

    # ── Synthetic fallback: plausible monthly arrivals 2000-2023 ──────────────
...
def conformal_interval(residuals: np.ndarray, preds: np.ndarray,
                       alpha: float = 0.05) -> tuple[np.ndarray, np.ndarray]:
    """
    Split conformal prediction intervals (log-space, then exponentiate).
    The finite-sample corrected quantile: ceil((n+1)(1-α)/n).
    """
    n      = len(residuals)
...
def fit_sarima_forecast(series_log: pd.Series, horizon: int,
                        alpha: float = 0.05) -> dict:
    """Fit auto_arima (SARIMA) and return point + conformal intervals."""
    if not HAS_PMDARIMA:
        return _dummy_forecast(series_log, horizon, "SARIMA", alpha)
    ...
    # ── Conformal calibration on trailing 24 months ───────────────────────────
    cal_len    = min(24, len(series_log) // 3)
    train_part = series_log.iloc[:-cal_len]
    cal_part   = series_log.iloc[-cal_len:]

    cal_model  = pm.auto_arima(
        train_part, seasonal=True, m=12,
        stepwise=True, error_action="ignore", suppress_warnings=True,
    )
    cal_preds  = cal_model.predict(n_periods=cal_len)
    residuals  = cal_part.values - cal_preds
...
def fit_prophet_forecast(series: pd.Series, horizon: int,
                         alpha: float = 0.05) -> dict:
    """Fit Prophet and return point + native prediction intervals."""
    if not HAS_PROPHET:
        return _dummy_forecast(series, horizon, "Prophet", alpha)
    ...
    future["covid"]   = 0  # no COVID in the forecast horizon
    ...
    return {
        ...
        "model":  "Prophet",
        "_prophet_obj": m,
        "_prophet_fcast": forecast,
    }


def _dummy_forecast(series, horizon: int, model_name: str,
                    alpha: float = 0.05) -> dict:
    """
    Naïve seasonal forecast (last-year same-month) used when pmdarima/prophet
    are not installed.  Uncertainty band = ±15% of point forecast.
    """
    vals   = np.log1p(series.values)
    preds  = []
    for h in range(horizon):
        lag = h + 12
        if lag < len(vals):
            preds.append(vals[-(lag)])
        else:
            preds.append(vals[-1])
...
def make_forecast_chart(hist: pd.Series, fcast: dict,
                        horizon: int, country: str,
                        show_history_months: int = 36) -> plt.Figure:
    """Return a matplotlib Figure with history + forecast + uncertainty band."""
    ...
    ax.scatter(fdates, fcast["point"] / 1_000,
...
def make_components_chart(fcast: dict, country: str) -> plt.Figure | None:
    """Return Prophet component chart if available."""
    if "_prophet_obj" not in fcast:
        return None
    m  = fcast["_prophet_obj"]
    fc = fcast["_prophet_fcast"]
    return m.plot_components(fc)


def make_interval_width_chart(fcast: dict, horizon: int) -> plt.Figure:
    """Bar chart showing prediction interval width across the forecast horizon."""
    fdates = pd.date_range(start=date.today(), periods=horizon, freq="MS")
    ...
    ax.set_xticklabels(
        [f"M+{i+1}" for i in range(horizon)], fontsize=8, rotation=45
    )
    ax.set_ylabel("Interval width (thousands)", fontsize=10)
    ax.set_title("Prediction interval width by horizon — wider = more uncertainty",
                 fontsize=11)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    fig.tight_layout()
...
def make_comparison_bar(comp_df: pd.DataFrame) -> plt.Figure:
    """Horizontal bar chart comparing model MAE from the Week 4 table."""
    ...
    if mae_col:
        mae    = pd.to_numeric(comp_df[mae_col], errors="coerce").tolist()
        colors = [PALETTE.get(str(m), "#999") for m in models]
        bars   = ax.barh(models, [m / 1_000 if m > 100 else m for m in mae],
        ...
        # No MAE column — show first numeric column instead
...
with st.sidebar:
    ...
    st.markdown("---")
    horizon = st.slider("**Forecast horizon (months)**", min_value=3, max_value=24, value=12, step=3)

    model_choice = st.radio(
        "**Forecasting model**",
        ["SARIMA + Conformal", "Prophet (native PI)", "Both"],
        index=0,
    )
    ...
    st.markdown("**Models:** ARIMA · SARIMA · Prophet · NeuralProphet")
...
if DATA_DIR.exists() and (DATA_DIR / "arrivals_clean.csv").exists():
    pass  # real data
else:
    st.markdown(
...
# ── Filter series for selected country ────────────────────────────────────────
country_df = (
...
# Remove COVID trough if present (optional — can comment out)
...
with col4:
    st.metric("Historical peak month",
              f"{peak_val/1e3:.0f}K",
...
with st.spinner(f"Fitting model(s) for {country}…"):
    if run_sarima:
        fcasts["SARIMA"] = fit_sarima_forecast(series_log, horizon, alpha)
    if run_prophet:
        fcasts["Prophet"] = fit_prophet_forecast(country_df, horizon, alpha)
...
if model_choice == "Both" and len(fcasts) == 2:
    # ── Side-by-side when both models are selected ────────────────────────────
...
# ── Model comparison table from Week 4 ────────────────────────────────────────
...
if comp_df is not None:
    ...
    with col_tbl:
        # highlight_min only on numeric columns that are actually present
    ...
    st.info(
        ...
        "Run the Week 4 notebook to generate `data/model_comparison_table.csv`."
    )

st.markdown("---")

# ── Test set predictions (post-COVID recovery) ─────────────────────────────────
...
st.caption(
    ...
    "Models: SARIMA (pmdarima), Prophet (Meta), NeuralProphet · "
